...
export default function ChapterContent({ content }: ChapterContentProps) {
  ...
  const handleTokenClick = (e: React.MouseEvent, token: TranslatedToken) => {
    e.stopPropagation()
    if (token.source === 'punct') return
    // Ignore click if the user was swiping or moving within the last 400ms
    if (hasMovedRef.current) return
    if (Date.now() - lastMoveTimestamp.current < 400) return
    // Ignore click if touch was held down for scrolling (> 450ms)
    if (touchStartPos.current && Date.now() - touchStartPos.current.time > 450) return
