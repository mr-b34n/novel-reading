...
export default function Reader() {
  ...
  // Feature: Auto-preload next chapter (+1) in the background for instant navigation & offline availability
  useEffect(() => {
    const nextIndex = currentChapter + 1
    if (nextIndex >= chapters.length) return
    const nextCh = chapters[nextIndex]
    if (!nextCh || !nextCh.sourceUrl) return

    // If next chapter content is not yet downloaded
    if (nextCh.content === '[Đang chờ tải...]' || !nextCh.content.trim()) {
      const timer = setTimeout(() => {
        fetchAliceChapterContent(nextCh.sourceUrl!)
          .then((res) => {
            if (res.content) {
              updateChapterContent(nextIndex, res.content)
            }
          })
          .catch((err) => {
            console.warn('Không thể tải trước chương sau:', err)
          })
      }, 600)
      return () => clearTimeout(timer)
    }
  }, [currentChapter, chapters, updateChapterContent])
