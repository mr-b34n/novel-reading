...
export async function updateBookChapter(
  ...
  if (!bookName || chapterIndex < 0) return
  try {
    const db = await getDB()
    const book: Book | undefined = await db.get(STORE_NAME, bookName)
    ...
    console.warn('Failed to update book chapter in IndexedDB:', err)
  }
}
